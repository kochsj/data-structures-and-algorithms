...
const evenOdd = (arr) => {
  return arr.map(value => {if(isNaN(value)){return 'N/A';} else if(value % 2 === 0){return 'even';} else {return 'odd';}});
};
...
const extractAbilities = (arr) => {
  return arr.map(abil => {return abil.ability.name});
};
